...
import { createLegacyBrowserSentencePlayer } from './runtime/browserSentencePlayer'
import {
...
const { remote } = usePlatformServices()
...
const player = useReadAloudSession(article, createConfiguredSentencePlayer(
  () => ttsSettings.value,
  {
    browserPlayer: createLegacyBrowserSentencePlayer(),
    remote,
  },
))
...
watch(readExpansionSettings, (nextSettings, previousSettings) => {
  const nextAi = nextSettings.ai
  const previousAi = previousSettings?.ai
  const providerConfigurationChanged = !previousAi
    || nextAi.enabled !== previousAi.enabled
    || nextAi.openai.apiKey !== previousAi.openai.apiKey
    || nextAi.openai.baseUrl !== previousAi.openai.baseUrl
    || nextAi.openai.model !== previousAi.openai.model
  if (providerConfigurationChanged) {
    readExpansionAiStates.value = {}
  }
  if (!nextAi.enabled || !nextAi.openai.apiKey.trim()) {
    showAiExpansionConsent.value = false
    pendingAiExpansionTerm = null
  }
})
  ...
  <main class="app-shell">
    ...
    <template v-else-if="article">
      ...
      <CompletionPanel
        ...
        v-model:remember-open-ai-key="rememberOpenAiKey"
        :can-remember-on-device="canRememberOnDevice"
        :article="article"
